...
class GATv2Conv(MessagePassing):
    ...
    def forward(self, x: Union[Tensor, PairTensor], edge_index: Adj,
                edge_weight: Tensor, h0: Union[Tensor, None], beta: float,
                delta: float, size: Size = None, return_attention_weights: bool = None):
        ...
        if isinstance(x, Tensor):
            assert x.dim() == 2
            x_l = self.lin_l(x) * beta + (1-beta) * x
            x_l = x_l.view(-1, H, C)
            if self.share_weights:
                x_r = x_l
            else:
                x_r = self.lin_r(x) * beta + (1-beta)*x
                x_r = x_r.view(-1, H, C)
        else:
            x_l, x_r = x[0], x[1]
            assert x[0].dim() == 2
        ...
        if self.add_self_loops:
            if isinstance(edge_index, Tensor):
                ...
                edge_index, edge_weight = add_self_loops(edge_index, edge_weight=edge_weight,
                                               num_nodes=num_nodes)
            elif isinstance(edge_index, SparseTensor):
                edge_index = set_diag(edge_index)

        # propagate_type: (x: PairTensor)
        out = self.propagate(edge_index, edge_weight=edge_weight, x=(x_l, x_r), size=size)
    ...
    def message(self, x_j: Tensor, x_i: Tensor, index: Tensor, ptr: OptTensor, edge_weight:Tensor,
        ...
        x_j = edge_weight.view(-1 , 1 , 1) * x_j
