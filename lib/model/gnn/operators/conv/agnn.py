...
class AGNNConv(MessagePassing):
    ...
    def forward(self , x, edge_index: Adj , edge_weight: Tensor , h0, beta: float ,
                delta: float):
        ...
        support = self.propagate(edge_index, edge_weight=edge_weight , x=x, x_norm=x_norm, size=None)
        if h0 is not None:
            output = delta * support + (1-delta) * h0
        else:
            output = support
        return output


    def message(self, x_j: Tensor, edge_weight: Tensor, x_norm_i: Tensor, x_norm_j: Tensor,
